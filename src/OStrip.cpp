...
OStrip::OStrip(const OStrip& orig) {
}

OStrip::~OStrip() {
}

void OStrip::set_view_type(VIEW_TYPE view_type, CHANNEL_TYPE channel_type) {
...
void OStrip::init(int index, OAlsa* alsa, Gtk::Window * wnd) {
    ...
    m_Pan[0]->signal_value_changed.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_Pan[0]));
    m_Pan[0]->set_hexpand(false);
    m_Pan[0]->set_halign(Gtk::ALIGN_CENTER);

    m_MuteEnable = &wnd_->m_MuteEnable[index];
    m_MuteEnable->set_value(alsa->getBoolean(CTL_NAME_MUTE, index) ? 1 : 0);
    m_MuteEnable->signal_switched.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_MuteEnable));

    m_SoloEnable = &wnd_->m_SoloEnable[index];
    m_SoloEnable->set_value(alsa->getBoolean(CTL_NAME_SOLO, index) ?  1 : 0);
    m_SoloEnable->signal_switched.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_SoloEnable));

    m_PhaseEnable[0] = &wnd_->m_PhaseEnable[index];
    m_PhaseEnable[0]->set_value(alsa->getBoolean(CTL_NAME_PHASE, index) ?  1 : 0);
    m_PhaseEnable[0]->signal_switched.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_PhaseEnable[0]));


    if (!(index % 2)) {
        m_Pan[1] = &wnd_->m_Pan[index + 1];
        m_Pan[1]->set_value(alsa->getInteger(CTL_NAME_PAN, index + 1));
        m_Pan[1]->signal_value_changed.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_Pan[1]));
        m_Pan[1]->set_hexpand(false);
        m_Pan[1]->set_halign(Gtk::ALIGN_CENTER);
        m_PhaseEnable[1] = &wnd_->m_PhaseEnable[index + 1];
        m_PhaseEnable[1]->set_value(alsa->getBoolean(CTL_NAME_PHASE, index + 1) ? 1 : 0);
        m_PhaseEnable[1]->signal_switched.connect(sigc::bind<>(sigc::mem_fun(wnd_, &OMainWnd::on_control_changed), m_PhaseEnable[1]));
    }
}

void OStrip::reset(OAlsa* alsa, int index) {
...
void OStrip::save_values(FILE * file) {
    ...
    fprintf(file, "\t\t<mute>");
    fprintf(file, "%d", (int) m_MuteEnable->get_value());
    fprintf(file, "</mute>\n");

    fprintf(file, "\t\t<phase>");
    fprintf(file, "%d", (int) m_PhaseEnable[0]->get_value());
    fprintf(file, "</phase>\n");

}

void OStrip::load_values(Glib::ustring xml) {
    ...
    try {
        ...
        while (reader.read()) {
            ...
            if (!strcmp(reader.get_name().c_str(), "mute") && reader.get_node_type() != XML_ENDELEMENT) {
                reader.read();
                m_MuteEnable->set_value(atoi(reader.get_value().c_str()));
                usleep(RESET_VALUE_DELAY);
            }

            if (!strcmp(reader.get_name().c_str(), "phase") && reader.get_node_type() != XML_ENDELEMENT) {
                reader.read();
                m_PhaseEnable[0]->set_value(atoi(reader.get_value().c_str()));
                usleep(RESET_VALUE_DELAY);
            }
        }
